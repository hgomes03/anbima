...
public class SelicController {
	...
	@ResponseStatus(HttpStatus.OK)
	@RequestMapping(value= "/media/{ano}", method = RequestMethod.GET, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResourceDTO selic(@PathVariable String ano) throws NotFoundException, ConstraintException {
		
		if (ano!=null) {
			this.selicServico.validate(ano, null);
		} 
		
		List<Selic> selics = selicServico.findByData(ano, null);
		
		MediaDTO media = selicServico.calcularMedia(ano, selics);
